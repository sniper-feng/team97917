...
void auto_role_one_1_(){
 ...
 timer time;
 time.reset();
  moveByEncoder(80, 0, 1000);
  mtr_push.spinFor(directionType::fwd, 3, rotationUnits::rev);
  mtr_push.spinFor(directionType::fwd, -3, rotationUnits::rev);
  mtr_intake_L.spin(fwd, 1000, voltageUnits::mV);
    mtr_intake_R.spin(fwd, 1000, voltageUnits::mV);
  intake_full_power_in();
  spinByAbsoluteAngle(100, 10000);
}