...
void moveByEncoder(int speed , int turn , int enc_target , int timeOutMilli){
  ...
  while(abs(get_move_enc()-temp) <= abs(enc_target) && time.time(msec) < timeOutMilli)
  move_set_speed(enc_target>0?speed:-speed, turn);
  move_stop();
}
...
void spinByEncoder_degree(int degree,int speed,int timeOutMilli){
  double temp = gyro_z.value(rotationUnits::deg);
  int target;
  double wheelbase = 28*1.37;
  double Wheeldiameter = 10.16;
  target = wheelbase/Wheeldiameter*degree*(900/360);
  spinByEncoder_target(target,speed,timeOutMilli);
  Brain.Screen.printAt(0,170," delta Angle : %.3f",gyro_z.value(rotationUnits::deg)-temp);
}
...
void pushToMaxium(){
  ...
  int temp = get_push_enc();
  volatile int refresh = 0;
  thread([](){
    timer time;
    while(time.time(msec) < 30);
    while(time.time(msec)< 300)handle_intake(false, true, false);
  }).detach();
  while(runtime.time(msec) < 7000){
    //anoymous lambda to control the intake
    ++refresh;
  handle_push(true,false,false);
  if(refresh > 1000){
    if(abs(temp - get_push_enc()) < 5)break;
    else
    temp = get_push_enc();
  }
  }
...
static bool usingSlow = false;
static int fixingBase = 0;
static const double kp = -200;//TEST!!!!
static int err_swing = 0,output_swing = 0;
void intakeSpinIn(){
  fixingBase = get_swing_enc();
  while(1){
    handle_swing(0,false);//这里只是为了使用函数里的限位器
  handle_intake(false, true, usingSlow);
  swing_set_voltage(-12000);
  }
}
void intakeSpinOut(){
  fixingBase = get_swing_enc();
  while(1){
  handle_intake(true, false, usingSlow);
  // This part is to lock the swing
  err_swing = get_swing_enc() - fixingBase;
      output_swing = err_swing * kp;
      if(output_swing > 12800)output_swing =12800;
      if(output_swing<-12800)output_swing = -12800;
    swing_set_voltage(output_swing);
  }
}
static vex::thread intakeThread;
void enableIntake_In(bool isSlowMode){
  usingSlow = isSlowMode;
